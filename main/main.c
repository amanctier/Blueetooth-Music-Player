...
#define WAV_FILE_PATH "/sdcard/pit.wav"
enum {
...
// SD Card and WAV file variables
static FILE *wav_file = NULL;
static wav_header_t wav_header;
static bool wav_file_ready = false;
...
static esp_err_t parse_wav_header(FILE *file, wav_header_t *header);
static bool open_wav_file(void);
...
/* utils for transfer BLuetooth Device Address into string form */
...
static esp_err_t init_sd_card(void)
{
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(BT_AV_TAG, "Failed to initialize SPI bus: %s", esp_err_to_name(ret));
        return ret;
    }
    // 2) SDSPI host with limited clock
    sdmmc_host_t host = SDSPI_HOST_DEFAULT();
    host.slot = SPI2_HOST;
    host.max_freq_khz = 15000;  // reduce clock to improve CRC margin
    // 3) SD SPI device (slot)
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(BT_AV_TAG, "Failed to mount SD card: %s", esp_err_to_name(ret));
        if (ret == ESP_FAIL) {
            ESP_LOGE(BT_AV_TAG, "Failed to mount filesystem. Set format_if_mount_failed = true to auto-format.");
        }
        return ret;
    }
    ESP_LOGI(BT_AV_TAG, "SD card mounted successfully");
    if (card) sdmmc_card_print_info(stdout, card);
    return ESP_OK;
}
static esp_err_t parse_wav_header(FILE *file, wav_header_t *header)
{
    uint8_t wav_header_buffer[44];
    
    if (fread(wav_header_buffer, 1, 44, file) != 44) {
        ESP_LOGE(BT_AV_TAG, "Failed to read WAV header");
        return ESP_FAIL;
    }
    // Check RIFF header
    if (memcmp(wav_header_buffer, "RIFF", 4) != 0) {
        ESP_LOGE(BT_AV_TAG, "Invalid WAV file: missing RIFF header");
        return ESP_FAIL;
    }
    // Check WAVE format
    if (memcmp(wav_header_buffer + 8, "WAVE", 4) != 0) {
        ESP_LOGE(BT_AV_TAG, "Invalid WAV file: missing WAVE format");
        return ESP_FAIL;
    }
    // Extract audio parameters
    header->num_channels = *(uint16_t*)(wav_header_buffer + 22);
    header->sample_rate = *(uint32_t*)(wav_header_buffer + 24);
    header->bits_per_sample = *(uint16_t*)(wav_header_buffer + 34);
    header->data_size = *(uint32_t*)(wav_header_buffer + 40);
    ESP_LOGI(BT_AV_TAG, "WAV file info:");
    ESP_LOGI(BT_AV_TAG, "  Channels: %d", header->num_channels);
    ESP_LOGI(BT_AV_TAG, "  Sample rate: %lu Hz", header->sample_rate);
    ESP_LOGI(BT_AV_TAG, "  Bits per sample: %d", header->bits_per_sample);
    ESP_LOGI(BT_AV_TAG, "  Data size: %lu bytes", header->data_size);
    // Validate audio format for A2DP (typically 44.1kHz, 16-bit, stereo)
    if (header->sample_rate != 44100 || header->bits_per_sample != 16 || header->num_channels != 2) {
        ESP_LOGW(BT_AV_TAG, "WAV format may not be optimal for A2DP streaming");
        ESP_LOGW(BT_AV_TAG, "Recommended: 44.1kHz, 16-bit, stereo");
    }
    return ESP_OK;
}
static bool open_wav_file(void)
{
    ...
    if (wav_file == NULL) {
        ESP_LOGE(BT_AV_TAG, "Failed to open WAV file: %s", WAV_FILE_PATH);
        return false;
    }
    if (parse_wav_header(wav_file, &wav_header) != ESP_OK) {
        fclose(wav_file);
        wav_file = NULL;
        return false;
    }
    wav_file_ready = true;
    ESP_LOGI(BT_AV_TAG, "WAV file opened successfully");
    return true;
}
/*********************************
...
static void bt_av_hdl_stack_evt(uint16_t event, void *p_param)
{
    ...
    case BT_APP_STACK_UP_EVT: {
        ...
        do {
            int tmr_id = 0;
            s_tmr = xTimerCreate("connTmr", (400 / portTICK_PERIOD_MS),
...
/* Modified to stream WAV file data instead of random noise */
static int32_t bt_app_a2d_data_cb(uint8_t *data, int32_t len)
{
    if (data == NULL || len < 0) {
        return 0;
    }
    if (!wav_file_ready || wav_file == NULL) {
        ESP_LOGW(BT_AV_TAG, "WAV file not ready, attempting to open...");
        if (!open_wav_file()) {
            // Fall back to silence if no file
            memset(data, 0, len);
            return len;
        }
    }
    size_t bytes_read = fread(data, 1, len, wav_file);
    
    if (bytes_read < len) {
        ESP_LOGI(BT_AV_TAG, "End of WAV file reached, looping...");
        // Rewind to start of audio data (skip 44-byte header)
        fseek(wav_file, 44, SEEK_SET);
        
        // Fill remaining buffer with data from beginning of file
        size_t remaining = len - bytes_read;
        size_t additional_bytes = fread(data + bytes_read, 1, remaining, wav_file);
        bytes_read += additional_bytes;
    }
    if (bytes_read == 0) {
        ESP_LOGW(BT_AV_TAG, "No data read from WAV file");
...
static void bt_app_av_media_proc(uint16_t event, void *param)
{
    ...
    case APP_AV_MEDIA_STATE_STARTED: {
        if (event == BT_APP_HEART_BEAT_EVT) {
            /* Modified: Continue streaming instead of stopping after 10 intervals */
            if (++s_intv_cnt >= 10) {
                s_intv_cnt = 0; // Reset counter to continue streaming
                ESP_LOGI(BT_AV_TAG, "a2dp media streaming continues...");
            }
        }
        break;
    }
...
void bt_av_notify_evt_handler(uint8_t event_id, esp_avrc_rn_param_t *event_parameter)
{
    ...
    uint8_t vol = event_parameter->volume; // 0..127
    ESP_LOGI(BT_RC_CT_TAG, "Volume changed (sink): %u", vol);
    // Do NOT call set_absolute_volume here to avoid echo loop.
    bt_av_volume_changed();  // re-register RN if supported
    break;
}
    /* other */
    default:
...
void app_main(void)
{
    ...
    ret = init_sd_card();
    if (ret != ESP_OK) {
        ESP_LOGE(BT_AV_TAG, "SD card initialization failed: %s", esp_err_to_name(ret));
        ESP_LOGE(BT_AV_TAG, "Continuing without SD card - will use silence");
    } else {
        ESP_LOGI(BT_AV_TAG, "SD card initialized successfully");
        // Try to open the WAV file
        if (!open_wav_file()) {
            ESP_LOGW(BT_AV_TAG, "Failed to open WAV file initially - will retry later");
        }
    }
    /*
    ...
    ESP_LOGI(BT_AV_TAG, "BR/EDR TX power set rc=%d", perr);
#if (CONFIG_EXAMPLE_SSP_ENABLED == true)
